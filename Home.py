# Home.py v1.2.3.4.2.1
"""
Weather Time v1.2.3.4.2.1 - Global Discover
- Recommendations: Added curated city buttons for quick navigation.
- Discover Engine: Categorized hubs (Global Capitals, Hidden Gems, Coastal).
- Logic: Refined city-switching state management to prevent search lag.
- Features: Retains v1.2.3.4.2 Comparison, Astro, and Comfort Score.
"""
...
st.markdown("""
    ...
    .stButton>button { width: 100%; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); }
    </style>
...
def fetch_full_intel(lat, lon):
    ...
    try: return requests.get(url).json(), requests.get(aqi_url).json()
...
with st.sidebar:
    ...
    recs = ["London", "Tokyo", "Dubai", "New York", "Paris", "Kandy", "Galle", "Nuwara Eliya"]
    
    # State handling for buttons
    if 'target_city' not in st.session_state:
        st.session_state.target_city = "Colombo"

    for i, city in enumerate(recs):
        if rec_cols[i % 2].button(city):
            st.session_state.target_city = city

    st.divider()
    primary_city = st.text_input("Search Location", value=st.session_state.target_city)
    ...
    with t_sys:
        if os.path.exists("changelog.json"):
            with open("changelog.json", "r") as f:
                ch = json.load(f)
                for entry in ch:
                    with st.expander(f"v{entry['version']} - {entry['date']}", expanded=(entry['version']=="1.2.3.4.2.1")):
                        for note in entry['notes']: st.write(f"- {note}")

else:
    st.error("City not found.")

st.caption(f"v1.2.3.4.2.1 | Discover Release | {datetime.now().strftime('%H:%M:%S')}")